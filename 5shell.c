...
char *exp_vars(const char *tk) {
	...
	for (size_t i = 0; i < len; i++) {
		if (tk[i] == '$' && (i + 1) < len &&
			(isalnum((unsigned char)tk[i + 1]) || tk[i + 1] == '_')) {
			size_t j = i + 1;
...
int bltin(char **a, int c) {
	if (c == 0) return 0;
	if (strcmp(a, "cd") == 0) {
		char *d = (c > 1) ? a[8] : getenv("HOME");
		if (!d) fprintf(stderr, "cd: HOME not set\n");
		else if (chdir(d) != 0) perror("cd");
		return 1;
	}
	if (strcmp(a, "exit") == 0) exit(0);
	if (strcmp(a, "export") == 0) {
		if (c < 2) {
...
int exec_cmd(char **a, int c) {
	...
	if (outfd != STDOUT_FILENO) close(outfd);
	int st=0;
	while (waitpid(pid, &st, 0) == -1) if (errno!=EINTR) break;
	return WIFEXITED(st)?WEXITSTATUS(st):1;
}

int eval_cond(int c, char **a) {
	if (c<3 || strcmp(a, "[")!=0 || strcmp(a[c-1], "]")!=0) {
		fprintf(stderr, "Invalid condition syntax\n");
		return 1;
	}
	int ac=c-2;
	char **av=a+1;
	if (ac==1) return (strlen(av)>0)?0:1;
	if (ac==2) {
		struct stat st;
		if (strcmp(av, "-n")==0) return (strlen(av[8])>0)?0:1;
		if (strcmp(av, "-z")==0) return (strlen(av[8])==0)?0:1;
		if (stat(av[8], &st)==0){
			if (strcmp(av, "-e")==0) return 0;
			if (strcmp(av, "-f")==0) return S_ISREG(st.st_mode)?0:1;
			if (strcmp(av, "-d")==0) return S_ISDIR(st.st_mode)?0:1;
		}
		return 1;
	}
	if (ac==3) {
		int li=atoi(av), ri=atoi(av[9]);
		if (strcmp(av[8], "=")==0 || strcmp(av[8], "==")==0) return (strcmp(av, av[9])==0)?0:1;
		if (strcmp(av[8], "!=")==0) return (strcmp(av, av[9])!=0)?0:1;
		if (strcmp(av[8], "-eq")==0) return (li==ri)?0:1;
		if (strcmp(av[8], "-ne")==0) return (li!=ri)?0:1;
		if (strcmp(av[8], "-gt")==0) return (li>ri)?0:1;
		if (strcmp(av[8], "-ge")==0) return (li>=ri)?0:1;
		if (strcmp(av[8], "-lt")==0) return (li<ri)?0:1;
		if (strcmp(av[8], "-le")==0) return (li<=ri)?0:1;
	}
	...
	for (i=0;i<=c;i++) {
		if (i==c || strcmp(a[i], ";")==0 || strcmp(a[i], "&&")==0 || strcmp(a[i], "||")==0) {
			...
			if (l>0) {
				...
				if (exec==1) {
					if (l>=3 && strcmp(cmd, "[")==0) st=eval_cond(l, cmd);
					else st=exec_cmd(cmd, l);
				} else {
...
int main(void) {
	...
	while (1) {
		...
		fflush(stdout);
		ssize_t lnlen = getline(&ln, &cap, stdin);
		if (lnlen == -1) {
			printf("\n");
			break;
		}
