...
int eval_cond(int c, char **a) {
    fprintf(stderr, "eval_cond received %d tokens:\n", c);
    for (int i = 0; i < c; i++) {
        fprintf(stderr, "  '%s'\n", a[i]);
    }
    ...
    char **av = a + 1;
    if (ac == 1)
        return (strlen(av[0]) > 0) ? 0 : 1;
    if (ac == 2) {
        struct stat st;
        if (strcmp(av[0], "-n") == 0)
            return (strlen(av[1]) > 0) ? 0 : 1;
        if (strcmp(av[0], "-z") == 0)
            return (strlen(av[1]) == 0) ? 0 : 1;
        if (stat(av[1], &st) == 0) {
            if (strcmp(av[0], "-e") == 0)
